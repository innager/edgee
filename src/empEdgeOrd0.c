		...
		int *ncheck, double *lim, int *unb_mom, double *pval)
{
  ...
  double n, Cxy, A, bx, by, Bx, By, r, t, incr, xrtmp, coefn;
...
double gett(double *smp, double n, double coefn)
{
  ...
  m1 /= n;
  m2 = (m2 - n*pow(m1, 2))/(n - 1);
  return coefn*m1/sqrt(m2);
}

double gett2(double *smp, int nx, int ny, double coefn)
{
  int i;
  double mx1, my1, sumx2, sumy2, m2;
  ...
  mx1 /= nx;
  my1 /= ny;
  m2 = (sumx2 + sumy2 - nx*pow(mx1, 2) - ny*pow(my1, 2))/(nx + ny - 2);
  return coefn*(mx1 - my1)/sqrt(m2);
}
